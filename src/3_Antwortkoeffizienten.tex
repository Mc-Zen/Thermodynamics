...
\begin{align*}
    \frac{\diff V}{V}=\frac{1}{V}\left( \frac{\partial V}{\partial T}\right)_P\diff T+\frac{1}{V}\left( \frac{\partial V}{\partial P}\right)_T\diff P \equiv \alpha(T,P)\diff T-\kappa(T,P)\diff P.
\end{align*}
...
\end{align*}
obwohl $\udiff Q$ eigentlich ein unvollständiges Differential ist, sodass der letzte Term nur beschränkt gültig ist. 
Die insgesamt zugeführte Wärme lässt sich darüber nun leicht durch Aufintegration der Form
\begin{align*}
    Q(T)=\int_{T_0}^{T} \udiff Q = \int_{T_0}^{T}T' \diff S = \int_{T_0}^{T}\sm c_p\diff T'
\end{align*}
bestimmen.
...
\begin{align*}
    \boxed{U=\frac{f}{2}\sm RT\propto T}\:.
\end{align*}
...
Das dadurch eintretende \anf{Auftauen} der Freiheitsgrade kann ab entsprechenden Schwellenwerten festgestellt werden. Für unsere zwei Beispiele liegen die Auftaubereiche für Wasserstoff bei über \SI{1000}{\kelvin} für Oszillationen und bei \SI{200}{\kelvin} für Rotationen.
...
\end{align*}
\begin{formal}
    Das \formalemph{Gibbs'sche Theorem} besagt, dass sich die Gesamtentropie eines Systems gemischter idealer Gase als die Summe der Einzelentropien der Gase schreiben lässt.
\end{formal}
    ...
    \boxed{
        \begin{aligned}
            S & =\sum_j\sm _j\left(s_{j0}+\frac{f_j}{2}R\ln\frac{T}{T_0}+R\ln\frac{V}{\sm v_0}\right)-\sm R\sum_jx_j\ln x_j                 \\
              & =\sum_jS_j(\text{Einzelgase bei $T$ und }P\propto \frac{\sm _j}{v_j}=\frac{\sm }{V})+\text{Mischungsentropie $S_m(>0)$}
        \end{aligned}
    }\:.
\end{align*}
