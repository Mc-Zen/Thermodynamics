...
\begin{summary}
    ...
    Die \formalemph{Antwortkoeffizienten} beschreiben die Reaktion eines Systems unter festgelegten Zustandsänderungen.  Dabei unterscheidet man die \formalemph{isochoren} ($V=const$, $P/T=const$), \formalemph{isothermen} ($T=const$, $PV=const$), \formalemph{isobaren} ($P=const$, $V/T=const$) und \formalemph{isentropen} ($S=const$, $PV^{\gamma}=const$) Zustandsänderungen \textendash{} letztere umfassen auch adiabatische (ohne  Wärmeaustausch stattfindende) reversible ($\Delta S=0$) Prozesse. Nicht jeder adiabatischer Prozess ist jedoch isentrop.

    Die Beschreibung der relativen Volumenänderung eines Systems mit Kontrolle über Temperatur und Druck führt auf den \formalemph{thermischen Ausdehnungskoeffizienten} $\alpha$ und die \formalemph{isotherme Kompressibilität} $\kappa_T$:
    \begin{align*}
        \alpha=\frac{1}{V}\left( \frac{\partial V}{\partial T}\right)_P, \; \kappa_T=-\frac{1}{V}\left( \frac{\partial V}{\partial P}\right)_T.
    \end{align*} 
    Die Beschreibung der Entropieänderung bzw. des molaren quasistatischen Wärmeflusses eines Systems lässt auf die \formalemph{isobare molare spezifische Wärme} $c_p$,
    \begin{align*}
    ...
    \end{align*}
    schließen. Analog lässt sich unter Kontrolle von Temperatur und Volumen die \formalemph{isochore molare spezifische Wärme} $c_V$ definieren:
    \begin{align*}
    c_v=\frac{T}{\sm}\left(\frac{\partial S}{\partial T}\right)_V=\frac{1}{\sm}\left(\frac{\udiff Q}{\diff T}\right)_V.
    \end{align*}
    Dabei wurde die \formalemph{Enthalpie} als thermodynamisches Potential wie folgt eingeführt:
    \begin{align*}
        H=U+PV=H(T,P).
    \end{align*}
    ...
    \end{align*}
    verknüpft.
    Der \formalemph{Adiabatenexponent} $\gamma$ bildet das Verhältnis der spezifischen Wärmekapazitätskoeffizienten ab und geht für große Systeme mit entsprechend großen Freiheitsgraden $f$ gegen $1$:
    \begin{align*}
        \gamma=\frac{c_p}{c_v}=\frac{f+2}{f}.
    \end{align*}
    Die \formalemph{Maxwell-Beziehungen} beschreiben unter Nutzung der Integrabilitätsbedingung die Relationen zwischen den zweiten Ableitungen der inneren Energie mit:
    \begin{align*}
    ...
    \end{align*}
    Wir haben ferner das \formalemph{ideale Gas} eingeführt. Dessen Idealisierung besteht dabei in der Vernachlässigung der räumlichen Ausdehnung der \textendash{} und Wechselwirkungen zwischen den \textendash{} Gasteilchen.
    Das einkomponentige ideale Gas lässt sich mittels der \formalemph{idealen Gasgleichung} 
    \begin{align*}
        PV=\sm RT
    \end{align*}
    beschreiben. $R=8,3 mol^{-1}K^{-1}$ bezeichnet dabei die universelle Gaskonstante.

    Aus diesem Gesetz kann für konstante Temperatur das \formalemph{Boyle-Mariottsche-Gesetz} ($PV=const$), respektive das \formalemph{Gay-Lussacsche-Gesetz} ($VT^{-1}=const$) für konstanten Druck, abgeleitet werden.
    Ebenso grundlegend ist die \formalemph{kalorische Zustandsgleichung},
    \begin{align*}
        U=\frac{f}{2}\sm RT,
    \end{align*}
    welche sich aus dem \formalemph{Gleichverteilungssatz} der statistischen Mechanik für die Beschreibung der Energien pro Freiheitsgrad $f$ ergibt:
    \begin{align*}
        u=\frac{1}{2}RT.
    \end{align*}
    Die innere Energie eines Gases ist nur von dessen Temperatur abhängig. Dies kann mittels des \formalemph{Gay-Lussacschen-Überstromversuchs} demonstriert werden, welcher in einem abgeschlossenen System einen Gasstrom unter konstantbleibender Temperatur erzeugt.
    ...
    Wir haben einen Ausdruck für die molare Entropie eines einfomponentige idealen Gases mittels Integration hergeleitet:
    \begin{align*}
